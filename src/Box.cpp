...
bool Box::IntersectLocal(Ray3D& ray, HitResult& outHit, double tMin, double tMax) {
	...
	tvals[0] = (bounds[ray.sign[0]] - ray.start.x) * ray.invDir.x;
	tvals[1] = (bounds[1 - ray.sign[0]] - ray.start.x) * ray.invDir.x;
	...
	if (tymin > tvals[0]) tvals[0] = tymin;
	if (tymax < tvals[1]) tvals[1] = tymax;
	...
	if (tzmin > tvals[0]) tvals[0] = tzmin;
	if (tzmax < tvals[1]) tvals[1] = tzmax;
